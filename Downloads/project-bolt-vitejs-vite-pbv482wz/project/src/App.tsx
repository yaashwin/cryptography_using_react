...
const SECRET_KEY = "yaashwin";

function App() {
  const [plaintext, setPlaintext] = useState('');
  const [encryptedText, setEncryptedText] = useState('');
  const [decryptedText, setDecryptedText] = useState('');
  const [executionTime, setExecutionTime] = useState<number | null>(null); // Explicitly typing the state
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-100 to-primary-200 dark:from-gray-900 dark:to-gray-800 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        {(encryptedText || decryptedText || executionTime !== null) && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
            ...
            {executionTime !== null && (
              <div>
                <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Execution Time:</h3>
                <div className="bg-gray-100 dark:bg-gray-700 p-3 rounded-md">
                  <p className="text-sm text-gray-800 dark:text-gray-200">{executionTime} ms</p>
                </div>
              </div>
            )}
          </div>
